...
class BackupService
{
    public function createBackup()
    {
        ...
        // Add --no-tablespaces to fix "Access denied" error
        // Add --column-statistics=0 for MySQL 8 compatibility
        $command = sprintf(
            'mysqldump --no-defaults --no-tablespaces --column-statistics=0 --user="%s" %s --host="%s" --port="%s" "%s" > "%s"',
            $dbConfig['username'],
            $passwordPart,
            $dbConfig['host'],
            $dbConfig['port'],
        ...
        if ($returnVar !== 0) {
            // Fallback: Try without --column-statistics=0 if it fails (older MySQL versions)
             $command = sprintf(
                'mysqldump --no-defaults --no-tablespaces --user="%s" %s --host="%s" --port="%s" "%s" > "%s"',
                $dbConfig['username'],
                $passwordPart,
                $dbConfig['host'],
                $dbConfig['port'],
...
if (file_put_contents($servicePath, $content) !== false) {
    echo "BackupService updated successfully!\n";
} else {
    echo "Failed to update BackupService.\n";
}
